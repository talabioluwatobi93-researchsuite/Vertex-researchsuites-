...
export default function DefensePrepPage() {
  ...
  async function load() {
    ...
    if (!session.results) {
      setErrorMsg('Please complete your main analysis (Chapter 4) before preparing for your defense.')
      setLoading(false)
      return
    }
  ...
  async function handleGenerate() {
    ...
    try {
      const res = await fetch('/api/quantitative-analysis/defense-prep', {
        method: 'POST',
      ...
      if (!res.ok) {
        setErrorMsg(data.error || 'Defense prep generation failed.')
        setProcessing(false)
        setStage('offer')
        return
      }
  ...
  const grouped = qaList.reduce((acc: Record<string, QA[]>, qa) => {
      ...
      {stage === 'offer' && (
        <div style={{ backgroundColor: '#ffffff', borderRadius: '16px', padding: '24px', border: '1px solid #EEEEEE', textAlign: 'center' }}>
          ...
          <p style={{ fontSize: '13px', color: '#777777', marginBottom: '4px' }}>
            Likely questions your supervisor may ask, with answers built from your actual computed results.
          </p>
          ...
          >
            {processing ? 'Processing...' : `Generate Defense Prep${price > 0 ? ` (\u20A6${price.toLocaleString()})` : ''}`}
          </button>
        </div>
      )}
        ...
        <div>
          ...
          {Object.entries(grouped).map(([category, items]) => (
            <div key={category} style={{ marginBottom: '20px' }}>
              <p style={{ fontSize: '13px', fontWeight: 700, color: '#D4AF37', marginBottom: '10px', textTransform: 'uppercase', letterSpacing: '0.5px' }}>
                {category}
              </p>
              {items.map((qa, i) => {
                ...
                return (
                  <div
                    ...
                    style={{ backgroundColor: '#ffffff', borderRadius: '12px', padding: '14px 16px', border: '1px solid #EEEEEE', marginBottom: '10px', cursor: 'pointer' }}
                  >
                    ...
                    {isOpen && (
                      <p style={{ fontSize: '13px', color: '#555555', lineHeight: '1.6', marginTop: '10px', marginBottom: 0 }}>{qa.answer}</p>
                    )}
                  </div>
                )
              })}
