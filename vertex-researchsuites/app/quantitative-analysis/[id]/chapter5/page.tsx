...
export default function Chapter5Page() {
  ...
  function beginHold(readyAt: string | null, alreadyRevealed: boolean, content: string) {
    ...
    if (now >= target) {
      if (!alreadyRevealed) revealChapter5(content)
      else setStage('done')
      return
    }
  ...
  async function revealChapter5(content: string) {
    ...
    if (userData?.user?.id) {
      await supabase.from('bunker_items').insert({
        ...
        item_type: 'quantitative_analysis_chapter5',
  ...
  async function load() {
    ...
    const { data: session, error } = await supabase
      .from('quantitative_analysis_sessions')
      .select('chapter5_paid, chapter5_content, chapter5_ready_at, chapter5_revealed, results, research_framework')
      .eq('id', id)
      .single()
    ...
    if (!session.results) {
      setErrorMsg('Please complete your main analysis (Chapter 4) before adding Chapter 5.')
      setLoading(false)
      return
    }

    if (session.chapter5_paid && session.chapter5_content) {
        setChapter5Content(session.chapter5_content)
        setChapter5ReadyAt(session.chapter5_ready_at)
        beginHold(session.chapter5_ready_at, session.chapter5_revealed, session.chapter5_content)
        return
      }

    // simple auto-suggested limitations draft, based on the actual sample size — student edits freely
    const n = session.results?.sampleSize
    setLimitations(
      n
        ? `This study was limited to a sample of ${n} respondents, which may affect the generalizability of the findings to a wider population. The data was collected through self-reported responses, which may be subject to social desirability bias. Time and resource constraints also limited the scope of the study.`
        : ''
    )
  ...
  async function handleGenerate() {
    if (!problemStatement.trim() || !methodology.trim() || !limitations.trim()) {
      setErrorMsg('Please fill in all three fields before continuing.')
      return
    }
    ...
    // deduct from wallet — placeholder ledger deduction until a real payment gateway is wired in
    if (price > 0) {
      const { data: userData } = await supabase.auth.getUser()
    ...
    const chapter5_inputs = { problemStatement, methodology, limitations }

    await supabase
      .from('quantitative_analysis_sessions')
      .update({ chapter5_inputs })
      .eq('id', id)

    setStage('generating')

    try {
      const res = await fetch('/api/quantitative-analysis/chapter5', {
        method: 'POST',
      ...
      if (!res.ok) {
        setErrorMsg(data.error || 'Chapter 5 generation failed.')
        setProcessing(false)
        setStage('form')
        return
      }
      setChapter5Content(data.chapter5_content)
      setChapter5ReadyAt(data.chapter5_ready_at)
      beginHold(data.chapter5_ready_at, false, data.chapter5_content)
    } catch (e: any) {
      setErrorMsg('Something went wrong generating Chapter 5.')
      setStage('form')
    }
    setProcessing(false)
  }
    ...
    <div style={{ maxWidth: '600px', margin: '0 auto', padding: '24px 16px' }}>
      {stage === 'offer' && (
        <div style={{ backgroundColor: '#ffffff', borderRadius: '16px', padding: '24px', border: '1px solid #EEEEEE', textAlign: 'center' }}>
          ...
          <p style={{ fontSize: '13px', color: '#777777', marginBottom: '4px' }}>
            Summary, Conclusion, Limitations & Recommendations &mdash; built directly from your Chapter 4 findings.
          </p>
          <p style={{ fontSize: '20px', fontWeight: 700, color: '#D4AF37', margin: '16px 0' }}>
            {price === 0 ? 'Free' : `\u20A6${price.toLocaleString()}`}
          </p>
          {errorMsg && (
            <div style={{ backgroundColor: '#FDEDEC', borderRadius: '12px', padding: '12px', marginBottom: '16px' }}>
              <p style={{ color: '#C0392B', fontSize: '12px', margin: 0 }}>{errorMsg}</p>
            </div>
          )}
          <button
      ...
      {stage === 'form' && (
        <div>
          ...
          <p style={{ fontSize: '12px', color: '#777777', marginBottom: '20px' }}>
            These help us write a Chapter 5 that fits your specific study.
          </p>
          ...
          <div style={{ backgroundColor: '#ffffff', borderRadius: '16px', padding: '16px', border: '1px solid #EEEEEE', marginBottom: '14px' }}>
            ...
            <textarea
              ...
              placeholder="What gap or problem does your study address?"
              rows={4}
              style={{ width: '100%', padding: '10px', borderRadius: '8px', border: '1px solid #EEEEEE', fontSize: '13px', color: '#333333', boxSizing: 'border-box', fontFamily: 'inherit' }}
            />
          </div>
          ...
          <div style={{ backgroundColor: '#ffffff', borderRadius: '16px', padding: '16px', border: '1px solid #EEEEEE', marginBottom: '14px' }}>
            ...
            <textarea
              ...
              placeholder="Briefly describe your research design, population, sampling technique, and instrument."
              rows={4}
              style={{ width: '100%', padding: '10px', borderRadius: '8px', border: '1px solid #EEEEEE', fontSize: '13px', color: '#333333', boxSizing: 'border-box', fontFamily: 'inherit' }}
            />
          </div>
          ...
          <div style={{ backgroundColor: '#ffffff', borderRadius: '16px', padding: '16px', border: '1px solid #EEEEEE', marginBottom: '16px' }}>
            ...
            <p style={{ fontSize: '11px', color: '#777777', marginBottom: '8px' }}>We drafted a starting point based on your data &mdash; edit freely.</p>
            <textarea
              value={limitations}
          ...
          <button
            ...
            style={{ width: '100%', backgroundColor: '#D4AF37', color: '#333333', border: 'none', borderRadius: '10px', padding: '14px', fontSize: '14px', fontWeight: 700, cursor: 'pointer' }}
          >
            {processing ? 'Processing...' : `Generate Chapter 5${price > 0 ? ` (\u20A6${price.toLocaleString()})` : ''}`}
          </button>
        </div>
      )}
      ...
      {stage === 'generating' && (
        <div style={{ padding: '60px 20px', textAlign: 'center' }}>
          <p style={{ color: '#777777', fontSize: '14px' }}>Writing your Chapter 5...</p>
        </div>
      )}
