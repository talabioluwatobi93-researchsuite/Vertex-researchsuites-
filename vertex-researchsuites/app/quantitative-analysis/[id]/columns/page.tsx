...
export default function QuantColumnsPage() {
  ...
  const [newConstructRole, setNewConstructRole] = useState<'IV' | 'DV' | 'Demographic'>('IV')
  ...
  const toggleReverse = (colIndex: number, constructId: string) => {
    setConstructs((prev) =>
      prev.map((c) => {
        ...
        return { ...c, reverseIndexes: isReversed ? c.reverseIndexes.filter((i) => i !== colIndex) : [...c.reverseIndexes, colIndex] }
      })
    )
  }
  ...
  const roleColor = (role: string) => (role === 'IV' ? '#D4AF37' : role === 'DV' ? '#2E7D32' : '#777777')
  ...
  return (
    <div style={{ backgroundColor: '#F9F9F9', minHeight: '100vh', padding: '24px 20px' }}>
      ...
      <p style={{ color: '#777777', fontSize: '13px', marginBottom: '20px' }}>
        Step 3 of 10 &mdash; Create constructs, tag each as IV, DV, or Demographic, then assign columns.
      </p>
      ...
      <div style={{ backgroundColor: '#ffffff', borderRadius: '16px', padding: '16px', border: '1px solid #EEEEEE', marginBottom: '16px' }}>
        ...
        <div style={{ display: 'flex', gap: '8px', marginBottom: '10px' }}>
          ...
          <select
            ...
            onChange={(e) => setNewConstructRole(e.target.value as 'IV' | 'DV' | 'Demographic')}
            style={{ padding: '8px', borderRadius: '8px', border: '1px solid #EEEEEE', fontSize: '12px' }}
          >
