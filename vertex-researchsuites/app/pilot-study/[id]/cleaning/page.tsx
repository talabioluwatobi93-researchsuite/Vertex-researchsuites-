...
export default function PilotStudyCleaningPage() {
  const { id } = useParams()
  ...
  async function loadSession() {
    ...
    // 3. Straight-lining detection (same value across every item in a construct with 3+ items)
  ...
  async function handleContinue() {
    ...
    const cleaning_config = {
      scaleMin: session?.cleaning_config?.scaleMin ?? 1,
      scaleMax: session?.cleaning_config?.scaleMax ?? 5,
    ...
    router.push(`/pilot-study/${id}/results`)
  }
  ...
  return (
    <div style={{ maxWidth: '700px', margin: '0 auto', padding: '24px 16px' }}>
      ...
      <div style={{ backgroundColor: '#ffffff', borderRadius: '16px', padding: '16px', border: '1px solid #EEEEEE', marginBottom: '16px' }}>
        ...
        {scaleConstructs.length === 0 && <p style={{ fontSize: '13px', color: '#777777' }}>No constructs found for this session.</p>}
        {scaleConstructs.map((c: any) => {
