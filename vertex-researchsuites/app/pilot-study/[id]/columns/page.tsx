...
export default function PilotStudyColumnsPage() {
  ...
  return (
    <div style={{ backgroundColor: '#F9F9F9', minHeight: '100vh', padding: '24px 20px' }}>
      ...
      <div style={{ backgroundColor: '#ffffff', borderRadius: '16px', padding: '16px', border: '1px solid #EEEEEE', marginBottom: '16px' }}>
            <p style={{ color: '#777777', fontSize: '12px', marginTop: '8px' }}>Your scale: {scaleMin} to {scaleMax} ({scaleMax - scaleMin + 1} points)</p>
        ...
        <div style={{ display: 'flex', gap: '8px', marginBottom: '10px' }}>
          ...
          {constructs.map((c) => (
            <span key={c.id} style={{ display: 'inline-flex', alignItems: 'center', gap: '6px', backgroundColor: '#F9F9F9', border: '1px solid #EEEEEE', borderRadius: '20px', padding: '4px 8px 4px 12px', fontSize: '12px', color: '#333333', marginRight: '6px', marginBottom: '6px' }}>
              ...
              <button type="button" onClick={() => deleteConstruct(c.id)} style={{ background: 'none', border: 'none', color: '#C0392B', cursor: 'pointer', fontSize: '14px', fontWeight: 700, padding: '0 4px', lineHeight: 1 }} aria-label={`Remove ${c.name}`}>
                ×
              </button>
            </span>
        ))}
